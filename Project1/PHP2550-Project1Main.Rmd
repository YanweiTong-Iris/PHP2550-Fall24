...
Performance distributions, as illustrated in **Figure 1**, were right-skewed across all age groups, with most runners finishing closer to the course record, and a long tail of slower times. This skewness is consistent across both genders, though differences in the central tendency of performance between men and women were observed within each age group. Given the non-normal distribution of net race times, log transformations will be necessary to normalize the data in linear-model analyses, ensuring more accurate modeling and interpretation of the factors influencing marathon performance. However, due to high non-linear relationship between the predictors and percent off course records or net race times (shown in RESULTS section), non-parametric models (e.g., LOESS and spline) will be used in subsequent analyses, so we will keep the outcomes on their original scales.
...
create_spline_plot <- function(df, show_y_axis =TRUE, 
  ...
  plot(spline_fit, xlab =" ", ylab = ylab_text, 
       main = x_label, cex.lab = 2, cex.main = 1.5)
