...
ggplot(merged_main %>% filter(Age >= 15), aes(x = SO2_s, y = X.CR, color = Gender)) +
  geom_point(size = 0.3, alpha = 0.3) +
  geom_smooth(method = "loess", se = TRUE, size = 0.75, color = "black") +
  facet_grid(Gender~Age_Group) +
  scale_color_manual(values = c("Male" = "steelblue", "Female" = "darkred")) + 
...
mtext("Figure: GAM Fits of Environmental Factors on % off Course Record", 
...
colnames(gam_results) <- c("Standardized var.", "n","Approxi. F-value",
                           "Approxi.p-value", "Adj. R2", "% Dev. explained")

gam_results %>%
  kable(caption = "GAM Results- % off Course Record vs. Standardized Environmental Factors",
        align = "c",
        escape = FALSE) %>%
