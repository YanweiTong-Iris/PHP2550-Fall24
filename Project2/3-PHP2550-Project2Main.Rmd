...
ROC_logistic = ggplot(roc_data, aes(x = 1-Specificity, y = Sensitivity)) +
  ...
  labs(
    title = "ROC for the Stepwise Model",
    x = "1 - Specificity",
    y = "Sensitivity"
  ) +
  theme_minimal() +
...
shared_title <- textGrob("Calibration Plots for Stepwise Model with Error Bars and LOESS", 
                         gp = gpar(fontsize = 14))

calibration_plots <- arrangeGrob(
  calib_error_bar, calib_loess,
  ncol = 2,
  top = shared_title
)

grid.arrange(
  calibration_plots, ROC_logistic,
  ncol = 2,
  widths = c(2, 1)
)
...
shared_title <- textGrob("Calibration Plots for Loass Model with Error Bars and LOESS", 
                         gp = gpar(fontsize = 14))
...
#remove intercept
optimal_coefs_enet <- as.numeric(coef(enet_model, s = best_lambda_lasso)[-1])
coef_names_enet <- rownames(coef(enet_model, s = best_lambda_lasso))[-1]  
...
kable(result_table_enet, 
      caption = "Elastic Net Model Coefficientsc at Optimal Lambda")
```
...
# Plot ROC curve and calculate AUC
# Generate ROC object
roc_obj <- roc(test_outcome, predicted_prob_enet)
...
ROC_enet = ggplot(roc_data, aes(x = 1-Specificity, y = Sensitivity)) +
  ...
  annotate("text", x = 0.8, y = 0.2, label = paste("AUC =", round(auc_value, 2)), size = 5, color = "Black") +
  labs(
    title = "ROC Curve for the Elastic Net Model",
    x = "1 - Specificity",
    y = "Sensitivity"
  ) +
  theme_minimal() +
...
shared_title <- textGrob("Calibration Plots for Elastic Net Model with Error Bars and LOESS", 
                         gp = gpar(fontsize = 14))
