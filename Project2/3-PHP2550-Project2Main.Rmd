...
For *Objective 1*, due to the complexity of the full model— which included a high number of potential interaction covariate terms— we employed the sequential replacement method. Sequential replacement is a more computationally efficient alternative to exhaustive search for complex models; it iteratively replaces variables to find a subset that provides a good balance between model fit and complexity. For *Objective 2*, the model was less complex as it included only baseline predictors without interaction terms. This allowed us to use an exhaustive search strategy, evaluating all possible subsets of the predictors to find the optimal model. 
...
```{r}
predictor_prob_best_subset <- as.numeric(predict_best_subset(test_data = test_predictors_dummy, 
                                   predictor_best_subset_coefs))-1
