...
The first row of **Figure 1** presents the relationship between the variance of the beta estimate and $G$ for different cost scenarios, with a fixed budget of $B = 4000$ and an outcome distribution following a normal model ($Y \sim \text{Normal}$). The black line indicates the variance trend, while the orange point represents the configuration (combination of $G$ and $R$) with the minimum variance. Each panel corresponds to a different combination of $c_1$ and $c_2$, showing the overall trend of variance reduction as the number of clusters ($G$) changes, and indicating the optimal design configuration. In all three panels, we observed a general trend where the variance of the $\hat{\beta}$ initially decreases as $G$ increases, reaches a minimum, and then fluctuates or remains relatively low at higher $G$ values. 

When $c_1 = 100$ and $c_2 = 10$, the number of clusters that minimizes variance is relatively high ($G = $). The higher $c_2$ implies that adding more observations within clusters is expensive, favoring an allocation with more clusters and fewer observations per cluster. With $c_1 = 100$ and $c_2 = 5$, the optimal $G$ is slightly lower ($G = $) since the reduced $c_2$ cost makes adding observations less expensive, but a high number of clusters is still favored. When $c_1 = 50$ and $c_2 = 10$, the optimal number of clusters increases further ($G = $) due to the lower cost per cluster, favoring designs with more clusters and fewer observations within each cluster.

The observed pattern indicates that the optimal design for minimizing variance depends heavily on the relative cost structure between $c_1$ and $c_2$. Lower $c_1/c_2$ ratios generally favor more clusters, whereas higher $c_2$ shifts the balance towards fewer clusters with potentially more observations. This highlights the trade-offs in resource allocation between expanding the number of clusters versus increasing the size of each cluster.
...
Looking at the overall pattern observed across the whole four rows of **Figure 1** reveals the relationship between different variance parameters ($\gamma^2$ and $\sigma^2$) and the optimal combination of $G$ and $R$ that minimize the variance of the treatment effect estimate ($\hat{\beta}$).

When $\gamma^2$ is higher, such as in the first and third rows, the optimal number of clusters tends to be higher. This is because between-cluster variability is greater, and having more clusters helps capture that variability effectively. Lower values of $\sigma^2$, indicating less within-cluster variance, generally result in a more precise estimate, as seen by the overall lower variance of $\hat{\beta}$ across the clusters in these scenarios.

Conversely, when $\sigma^2$ increases (seen in the third and fourth rows), the optimal $G$ becomes slightly lower compared to the scenarios with lower $\sigma^2$. The higher within-cluster variability makes it more efficient to allocate budget towards a balanced design between clusters and observations per cluster, as reflected in the reduced values of $G$.
...
**Figure 2** for the Poisson models exhibits a similar pattern where the optimal number of clusters ($G$) is highly sensitive to changes in the cost structure ($c_1$ and $c_2$). Generally, a lower cost per observation within a cluster ($c_2$) allows for a slight reduction in the optimal number of clusters while maintaining precision, favoring a higher $R$. Conversely, reducing the cost per cluster ($c_1$) typically leads to an increase in the optimal $G$, as more clusters can be sampled within the fixed budget. Additionally, when $\gamma^2$ is smaller, suggesting reduced between-cluster variability, there is a greater tendency to allocate resources to more clusters rather than more observations within each cluster. 

This overall pattern underscores the trade-off between capturing between-cluster variability effectively and optimizing resource allocation to minimize the variance of the treatment effect estimates. Compared to the Normal outcome models, the Poisson models show a greater degree of sensitivity to cost changes, and higher variability in optimal $G$. 


**Table 1** summarizes the performance measures of the optimal design across different data-generating settings for Normal and Poisson models. When the design is optimal—indicated by a relatively low variance of the estimated treatment effect ($Var(\hat{\beta})$)—the coverage probability and power are consistently high (both reaching 1 in almost all cases), which is an expected outcome. A lower variance means that the estimates are more precise, reducing uncertainty, which in turn leads to narrower confidence intervals that are more likely to contain the true effect, hence achieving high coverage. Additionally, with greater precision, the study is more likely to detect a true effect when it exists, resulting in high power. This relationship between low variance, high coverage, and high power aligns well with theoretical expectations in experimental design and statistical inference. Moreover, the emparical bias is minimal across the designs, supporting the theoretical property that $\hat{\beta}$ is unbiased.

A pattern emerges across different cost structures ($c_1$ and $c_2$) and variance components ($\gamma^2$ and $\sigma^2$). For instance, a lower within-cluster variance ($\sigma^2$) leads to smaller $Var(\hat{\beta})$, suggesting a higher precision in treatment effect estimates. When between-cluster variance ($\gamma^2$) is smaller, the optimal design typically involves more clusters ($G$) and fewer observations per cluster ($R$), optimizing the allocation of resources while maintaining precision. Changes in $c_1$ and $c_2$ also influence the design, with higher $c_2$ generally favoring fewer clusters and more observations within each cluster to minimize costs and achieve optimal variance. The trade-off between $G$ and $R$ appears to be well-balanced under these different scenarios, consistently maintaining low bias, high power, and sufficient coverage.
...
One limitation of this study is the computational constraint that prevented us from exploring all possible combinations of $c_1$, $c_2$ and the variance parameters $\sigma^2$ and $\gamma^2$ in greater detail. The simulation process required substantial computational resources, making it infeasible to comprehensively evaluate the impact of each parameter under all possible conditions. Additionally, the limited scope of parameter settings may restrict the generalizability of the findings across different experimental designs. Future studies could benefit from employing advanced computational techniques or parallel processing to more thoroughly investigate the impact of varying these design parameters.
