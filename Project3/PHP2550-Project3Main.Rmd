...
data_path = paste0(here::here(), "/simulated_data")
result_path = paste0(here::here(), "/results")
```
...
simulate_clustered_data <- function(model = "normal",
  ...
  Y <- unlist(Y_list)
  X_obs <- rep(X, each = R)
  data <- data.frame(Y, X_obs, cluster_id, observation_id)
  saveRDS(data, paste0("simulated_data_G", G, "_R", R, ".rds"))
  return(data)
}
...
#' @return 
find_optimal_G_R <- function(alpha, beta, gamma_sq, sigma_sq, B, c1, c2) {
  min_variance <- Inf
  best_G <- 0
  best_R <- 0
  ...
  for (G in 2:(B/c1)) {
    for (R in 5:20) {
      N <- G * R
      if (c1 * G + c2 * (N - G) <= B) {
        data <- simulate_clustered_data(model = "normal", alpha, 
                                        beta, gamma_sq, sigma_sq, 
                                        B, c1, c2, G, R)
        View(data)
        model <- lmer(Y ~ X_obs + (1 | cluster_id), data = data)
        print(summary(model))
        betahat <- fixef(model)["X_obs"])
        variance <- VarCorr(model)$cluster_id[1]
        
        if (is.na(betahat) || is.na(variance)) {
          cat("NA detected in betahat or variance for G =", G, "and R =", R, "\n")
          next
        }
        ...
        results <- rbind(results, data.frame(alpha = alpha, beta = beta, 
                                             gamma_sq = gamma_sq,sigma_sq = sigma_sq, 
                                             B = B, c1 = c1, c2 = c2,
                                             G = G, R = R, 
                                             betahat = betahat, variance = variance))
      }
    }
  }
  
  write.csv(results, 
            paste0("simulation_results_all_B_", B, "_c1_", c1, "_c2_", c2, ".csv"), 
            row.names = FALSE)
  return(results)
  cat("Minimum variance:", min_variance, "\n")
