...
**Budget constraint**: The budget constraint is defined as:

$$
c_1 + G(R-1)c_2 \leq B
$$
This reflects that each cluster incurs a higher cost for the initial observation ($c_1$), while subsequent observations within the same cluster are less costly ($c_2$).
...
Also, as of *Aim2*, to understand the impact of various parameters on the optimal design:
    ...
    3) **Evaluate Patterns and Relationships**:
      - Analyze how changes in cost structure and variance components contribute to changes in the optimal design. This helps us derive practical recommendations for designing cluster-randomized trials under varying budgetary and variance conditions.
...
find_optimal_G_R <- function(dist = "Normal", alpha, beta, gamma_sq, sigma_sq, B, c1, c2, n_simulations = 100) {
  ...
  for (G in 2:(floor((B / c1)) - 1)) {
    ...
    if (R > 1) {
      ...
      saveRDS(combined_data, paste0(subfolder,
...
var_vs_G_plot_by_facet <- function(results, group_name, dist) {
  ...
  if (dist == "Normal"){
    subtitle_text = bquote(list(B ==.(B), gamma^2 == .(gamma_sq), sigma^2 == .(sigma_sq), beta ==.(beta)))
  } else{
    subtitle_text = bquote(list(B ==.(B), gamma^2 == .(gamma_sq), beta ==.(beta)))
  }
  ...
  plot = ggplot(group_results, aes(x = G, y = betahat_var)) +
    ...
    geom_point(data = lowest_points, aes(x = G, y = betahat_var), color = "darkorange", size = 2) +
    geom_text(data = lowest_points, aes(x = G, y = betahat_var, 
                                        label = paste("Min. var =", round(betahat_var, 2), 
                                                      "\n at G =", G, ", R =", R)), 
              vjust = -1.2, hjust = 0.5 ,color = "darkorange", size = 3) +
    labs(
...
#foreach (i = 1:9, .packages = c("lme4")) %dopar%{
  ...
  Normal_results_list[[i]] <- result
}

Normal_combined_results <- do.call(rbind, Normal_results_list)
...
#foreach (i = 1:9, .packages = c("lme4")) %dopar%{
  ...
  # Store the result in the list
  Poisson_results_list[[i]] <- result
}

Poisson_combined_results <- do.call(rbind, Poisson_results_list)
