...
find_optimal_G_R <- function(dist = "Normal", alpha, beta, gamma_sq, sigma_sq, 
  ...
  for (G in 2:(floor((B / c1)) - 1)) {
    ...
    if (R > 1) {
      ...
      z_values <- numeric(n_simulations)
      ...
      for (sim in 1:n_simulations) {
        ...
        tryCatch({
          ...
          z_values[sim] <- coef(summary(model))["X_obs", "z value"]
        }, error = function(e) {
      ...
      valid_z_values <- z_values[!is.na(z_values)]
      ...
      if (length(betahats) > 0) {
        ...
        critical_z_value <- 1.96  # Approximate value for large n
        power <- mean(abs(valid_z_values) > critical_z_value, na.rm = TRUE)
